...
$(window).ready(function () {
        ...
        this.getmarks = function () {
            ...
            $.post("ajax/getNBCL.php", {
                ...
                for(var i = 0;i < that.marks.length;++i){
                    that.marks[i].print_mask_to = Fprint_mask_to;
                }
            })
        };
        function Fprint_mask_to(thatlabel){
    ...
    function findmask(notemask, maskid){
        masks = notemask.split('_');
    ...
    function ItemsContainer(notes, mark=0) {//将会接受label,books,items传来的参数,并根据情况输出给note_list还是起错名字了-_-
        ...
        this.getlist = function (reaction) {
            ...
            if (reaction === 'notes_all_show') {
                ...
                this.notes_info = findInfoFromArray(this.remark.marks, this.markid, 'markName');
                for (i = 0; i < this.notes.notes.length; ++i) {
                    if (this.notes.notes[i]['isdelete'] === '0' && findmask(notelist[i]['markID'], this.markid)) {
                        this.lists.push(this.notes.notes[i]);
                    }
                }
        ...
        this.listinit = function(reaction, need_id, autodisplay){
            ...
            this.changesort('updateDec', autodisplay);
        };
        this.displaylist = function(){//bookid, maskid这个只用管list notes
            $('#items_list').html('');
            if(this.isinit === 0){
                this.isinit += 1;
        ...
        this.lightnotes = function () {
            ...
            $('#items_list .glyphicon-star-empty').hover(function () {
                $(this).removeClass('glyphicon-star-empty').addClass('glyphicon-star');
            }, function () {
                $(this).removeClass('glyphicon-star').addClass('glyphicon-star-empty');
            });
        ...
        this.changesort = function(sortfor, autoDisplay){//updateAsc,updateDec,createAsc,updateDec,nameAsc,nameDec;
            ...
            if(sortfor === 'createAsc'){
                ...
                for(var i = 0;i < list.length; ++i){
                    // console.log(list[i].id + '|' + list[i].createtime + '|' + list[i].updatetime);
                }
                ...
                for(i = 0;i < list.length; ++i){
                    // console.log(list[i].id + '|' + list[i].createtime + '|' + list[i].updatetime + '|' + list[i].title);
                }
